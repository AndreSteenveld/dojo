...
var features = {
	// allow user to configure Dojo to use native QSA whenever a stable version is available
	"config-native-qsa": d.config.nativeQsa,
	// allow user to configure Dojo to use native QSA whenever a stable version with pseudo selector support (usually means reverting to our impl for IE)
	"config-native-qsa-with-pseudos": d.config.nativeQsaWithPseudos,
	"dom-qsa": qsaAvail,
	"bug-query-selector-ignores-caps": (function(){
		// NOTE(alex): the spec is idiotic. CSS queries should ALWAYS be case-sensitive, but nooooooo
		// this is taken from has.js repository:
		var e2, buggy = null, el = getDoc().createElement("div");
		if(getDoc().compatMode == "BackCompat" && el.querySelector){
		    e2 = getDoc().createElement("span");
		    e2.className = "Test";
		    el.appendChild(e2);
	...
	"bug-query-selector-pseudo": dojo.isIE // TODO: convert to feature test
};
function has(feature){
	return features[feature];
}
var cssCaseBug = has("bug-query-selector-ignores-caps");
if(!has("dom-qsa") || !(has("config-native-qsa") || (has("config-native-qsa-with-pseudos") && !(has("bug-query-selector-pseudo"))))){
// we branch into the main query code if there is no QSA or if it is possibly buggy
	...
	*/
	...
	d._filterQueryResult = function(/*NodeList*/ nodeList, /*String*/ filter, /*String|DOMNode?*/ root){
		var tmpNodeList = new d._NodeListCtor(),
			...
			filterFunc =
				...
				function(node){
					return d.query(filter, root).indexOf(node) != -1;
				};
	...
	d.query = function(query, root){
		...
		var doc = rootIsDoc ? root : (root.ownerDocument||d.doc);
		// rewrite the query to be ID rooted
		if(!rootIsDoc || (">~+".indexOf(query.charAt(0)) >= 0)){
			root.id = root.id||("qUnique"+(ctr++));
			query = "#"+root.id+" "+query;
		}
		if(!rootIsDoc && !root.parentNode){
			doc.body.appendChild(root);
			var cleanupRoot = true;
		}
		...
		try{
			var results = doc.querySelectorAll(query);
			var array = [];
			for(var i = 0, l = results.length; i < l; i++){
				array.push(results[i]);
			}
			return d._NodeListCtor._wrap(array);
		}finally{
			if(cleanupRoot){
