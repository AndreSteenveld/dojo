(function(
  ...
  // 
  // This function defines a JavaScript script-inject loader--an AMD-compliant 
  // (http://wiki.commonjs.org/wiki/Modules/AsynchronousDefinition), requirejs-compatible 
  // (http://requirejs.org/) loader. 
  // 
  // This implementation is nearly identical with the bdLoad reference implementation; it contains the
  // following patches:
  // 
  //   * implements baseUrl sniffing, bracketed by has("dojo-sniff")
  //   * the bd.docGen function is removed
  // 
  // Tutorial resources available: 
  // 
  //   * For a complete tutorial on the use of this loader, see http://bdframework.org/docs/loader/loader.html.
  // 
  ...
  var 
    ...
    contextRequire= function(a1, a2, a3, referenceModule, contextRequire) {
      ...
      if (isArray(a1)) {
        ...
        for (var callback= a2, deps= [], mid, i= 0; i<a1.length;) {
          ...
          if (mid=="*ready") {
            callback= function() {
              for (var args= arguments, aargs= [], i= 0; i<args.length; aargs[i]= args[i++]){}
              req.ready(function() {
                a2.apply(null, aargs);
              });
            };
    ...
    makeModuleInfo= function(pid, mid, pqn, pack, path, url) {
      var result= {pid:pid, mid:mid, pqn:pqn, pack:pack, path:path, url:url, def:0};
      return result;
    },
    ...
    checkComplete= function() {
      ...
      signalFactoryRan= 1;
      while (signalFactoryRan) {
        signalFactoryRan= 0;
        for (var result, module, i= 0; i<execQ.length;) {
          module= execQ[i];
          result= execModule(module);
          if (module!==execQ[i]) {
            // execModule adjusted the execQ; therefore, restart the iteration
            signalFactoryRan= 1;
            break;
          }
          if (result!==abortExec) {
  ...
  if (has("loader-traceApi")) {
    ...
    // redefine nonModuleProps so it shows debugging nonmodule value
    nonModuleProps= {
      injected: arrived,
      deps: [],
      def: nonmodule,
  ...
  if (has("loader-injectApi")) {
    var
      injectPlugin= function(
        ...
        if (plugin.executed && !plugin.load) {
  ...
  var def= function(
    ...
    if (args[0]) {
      ...
      return;
    } // else anonymous module; therefore module id is implied by the resource being loaded

    if (has("dom-addEventListener") || !has("host-browser")) {
